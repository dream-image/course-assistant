...
const RoleTag = (props: { userInfo?: UserInfo, text?: string, icon?: string }) => {

  const { userInfo, text } = props
  if (text) {
    return (<>
      <Chip variant="faded"
        startContent={props.icon ? <Image src={props.icon} width={20} height={20}></Image> : null}
      >
...
const AvatarInfo = (props: { userInfo: UserInfo, hasIcon?: boolean, widthFlex?: boolean }) => {
    ...
    <>
      ...
      <div className={` h-full flex items-center ${widthFlex ? 'flex-1' : ''}`}>
        <div className='flex flex-col justify-center items-center'>
          <span className='text-[#1f1f1f] text-sm w-max'>{userInfo.nickname}</span>
